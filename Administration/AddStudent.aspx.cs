...
public partial class Admin_AddStudent : System.Web.UI.Page
{
    ...
    protected void FillClass()
    {
        ...
        {
            objData = new clsData();
            int resId = Convert.ToInt32(ddlResident.SelectedValue.ToString());
            DataTable Classdata = objData.ReturnDataTable("SELECT ClassId as Id,ClassName as Name  from [Class] where ActiveInd = 'A' AND ResidenceInd=" + resId + "", true);
    ...
    protected void FillClass1()
    {
        try
        {
            ...
            if (intStudentId > 0)
            {
                ...
                //int resId = Convert.ToInt32(ddlResident.SelectedValue.ToString());
                //DataTable dtClass = objData.ReturnDataTable("SELECT ClassId as Id,ClassName as Name  from [Class] where ActiveInd = 'A' AND ResidenceInd=" + resId + "", true);
                ...
                if (Dt.Rows.Count > 0)
                {
                    resident = Convert.ToInt32(Dt.Rows[0]["ResidenceInd"]);
                }
                DataTable Classdata = objData.ReturnDataTable("SELECT ClassId as Id,ClassName as Name  from [Class] where ActiveInd = 'A' AND ResidenceInd=" + resident + "", true);
                if (Classdata.Rows.Count > 0)
                {
                    DLclass.DataSource = Classdata;
                    DLclass.DataBind();
    ...
    protected void fillStudent()
    {
        if (intStudentId > 0)
        {
            ...
            DataTable Dt = objdataClass.fillData(strQuery);
            string StrClass = "SELECT stdCls.ClassId AS ClassId FROM StdtClass stdCls INNER JOIN Class Cls ON Cls.ClassId=stdCls.ClassId WHERE stdCls.StdtId=" + intStudentId + " AND Cls.ActiveInd='A' And stdCls.ActiveInd='A' ";
    ...
    protected void USaveClass(int stdtID, int classID) //fill StdtClass table
    {
        string strQuery = "Insert into StdtClass(SchoolId, StdtId, ClassId,PrimaryInd, ActiveInd, CreatedBy, CreatedOn,ModifiedBy,ModifiedOn) Values (" + sess.SchoolId + ", " + stdtID + ", " + classID + ",'A','A', '" + sess.LoginId + "' ,(SELECT Convert(Varchar,getdate(),100)),'" + sess.LoginId + "' ,(SELECT Convert(Varchar,getdate(),100)) )";
        int ClassId = objData.Execute(strQuery);
    }

    protected void updateclass(int stdtID)
    {
        try
        {
            clsClass objclass = new clsClass();
            ...
            if (sess != null)
            {
                if (stdtID > 0)
                {
                    foreach (DataListItem item in DLclass.Items)
                    {
                        ...
                        {
                            if (objclass.StudentClassExit(stdtID, Convert.ToInt32(hf.Value)) == false)
                            {
                                USaveClass(stdtID, Convert.ToInt32(hf.Value));
                            }
                            else
                            {
                                if (objclass.GetADClassStd(stdtID, Convert.ToInt32(hf.Value)) == "D")
                                {
                                    string strQuery = "Update StdtClass set ActiveInd='A' ,ModifiedBy=" + sess.LoginId + " ,ModifiedOn=(SELECT Convert(Varchar,getdate(),100)) where UserId=" + stdtID + " and ClassId=" + Convert.ToInt32(hf.Value) + "";
                        ...
                        {
                            if (objclass.StudentClassExit(stdtID, Convert.ToInt32(hf.Value)) == true)
                            {
                                if (objclass.GetADClassStd(stdtID, Convert.ToInt32(hf.Value)) == "A")
                                {
                                    string strQuery = "Update StdtClass set ActiveInd='D' ,ModifiedBy=" + sess.LoginId + " ,ModifiedOn=(SELECT Convert(Varchar,getdate(),100)) where StdtId=" + stdtID + " and ClassId=" + Convert.ToInt32(hf.Value) + "";
                                    objData.Execute(strQuery);
                                }
                            }
                        }
    ...
    protected void UpdateStudentAndAddress()
    {
        ...
        else if (txtMobile.Text != "" && clsGeneral.IsItValidPhone(txtMobile.Text.Trim()) == false)
        {
            ...
            {
                bool Residence = true;
                if (ddlResident.SelectedItem.Text.Trim() == "Day")
                    Residence = false;
                ...
                if (fileUpl_stdPhoto.HasFile)
                {
                    ...
                    if ((idIndex > 0) && (imageId != "defaultStudent.png") && (imageId != null) && imageId != "")
                    {
    ...
    protected void SaveClass(int StdtId, SqlConnection con, SqlTransaction Transs) //fill StdtClass table
    {
        foreach (DataListItem item in DLclass.Items)
        {
            CheckBox chklist = (CheckBox)item.FindControl("chkClass");
            ...
            if (chklist.Checked == true)
            {
                string strQuery = "Insert into StdtClass(SchoolId, StdtId, ClassId,PrimaryInd, ActiveInd, CreatedBy, CreatedOn,ModifiedBy,ModifiedOn) Values (" + sess.SchoolId + ", " + StdtId + ", " + Convert.ToInt16(hf.Value) + ",'A','A', '" + sess.LoginId + "' ,(SELECT Convert(Varchar,getdate(),100)),'" + sess.LoginId + "' ,(SELECT Convert(Varchar,getdate(),100)) )";
    ...
    protected void ddlResident_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlResident.SelectedIndex != 2)
            {
                objData = new clsData();
                int resId = Convert.ToInt32(ddlResident.SelectedValue.ToString());
                DataTable Classdata = objData.ReturnDataTable("SELECT ClassId as Id,ClassName as Name  from [Class] where ResidenceInd = " + resId + " and ActiveInd = 'A'", true);
                if (Classdata.Rows.Count > 0)
                {
                    DLclass.DataSource = Classdata;
                    DLclass.DataBind();
