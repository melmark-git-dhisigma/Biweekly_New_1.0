...
public partial class Admin_AddStudent1 : System.Web.UI.Page
{
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    if (sess.SchoolId == 1)
                    {
                        Response.Redirect("../Administration/Facesheet.aspx");
                    }
                    else if (sess.SchoolId == 2)
                    {
                        Response.Redirect("~/Administration/FacesheetView.aspx");
                    }
                }
            }
        }
    ...
    private void LoadData()
    {
        ...
        {
            intStudentId = Convert.ToInt32(sess.StudentId);
            fillStudent();
    ...
    protected void fillStudent()
    {
        ...
        //        ddlStatus.Visible = true;
            ...
            string strQuery = "SELECT   Std.ImageUrl,Std.StudentId, Std.ClassId, Std.StudentNbr, Std.StudentFname, Std.StudentLname,Std.GradeLevel,Std.ResidenceInd, Std.Gender,Std.JoinDt, Std.DOB,  " +
                          "Std.CreatedBy, Std.CreatedOn, Adr.AddressLine1, Adr.AddressLine2, Adr.AddressLine3, Adr.City, (select LookupName from lookup where LookupType='State'and LookupId=Adr.State) as 'State',Std.SASID,Std.DistrictCode,Std.DistrictType,Std.DistFunction,Std.DistContactPerson," +
                          "(select LookupName from lookup where LookupType='Country' and LookupId=Adr.Country) as 'Country', Adr.Zip, Adr.HomePhone, Adr.Mobile, Adr.Email, Adr.CreatedBy AS Expr1, Adr.CreatedOn AS Expr2 ,  " +
            ...
            try
            {
                ...
                {
                    Button_Add.Text = "UPDATE";
                    string Img = "../Administration/StudentsPhoto/" + Dt.Rows[0]["ImageUrl"].ToString();
                    ...
                    txtddlGender.Text = Dt.Rows[0]["Gender"].ToString();
                    ...
                    txtddlCountry.Text = Dt.Rows[0]["Country"].ToString().Trim();
                    ...
                    txtDateofLast.Text = Dt.Rows[0]["LastPhysicalDate"].ToString().Replace("-", "/");
                    ...
                    try
                    {
                       // objData.ReturnDropDown("SELECT LookupId as Id,LookupName as Name from LookUp where ParentLookupId = " + ddlCountry.SelectedValue + " AND LookupType = 'State'", ddlState);
                        txtddlState.Text = Dt.Rows[0]["State"].ToString().Trim();
                    }
                    catch (Exception Ex)
                    {
                    ...
                    if (resident == true)
                        txtddlResident.Text = "Resident";
                    else
                        txtddlResident.Text = "Non Resident";
                    sess.AdmStudentId = 0;
                    Session["Status"] = null;
    ...
    protected void SaveStudent()
    {
        ...
        try
        {
            ...
            //objData.CommitTransation(Transs);
